...
// Component for typing dots animation
const AnimatingDots = () => {
  const [visibleDots, setVisibleDots] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setVisibleDots((prev) => (prev + 1) % 4); // Cycle through 0, 1, 2, 3 dots
    }, 300); // Faster typing - change every 300ms instead of 500ms

    return () => clearInterval(interval);
  }, []);

  return (
    <span className="inline-block w-[18px]"> {/* Fixed width to prevent layout shift */}
      {Array.from({ length: visibleDots }, (_, i) => '.').join('')}
    </span>
  );
};

// Memoized cell components to prevent animation interruption
const AnimatedCell = React.memo(({ 
  ...
  const thinkingSteps = [
    <span key="chunking" style={{ color: '#8F8C85' }}>
      Chunking content<AnimatingDots />
    </span>,
    <span key="embedding" style={{ color: '#8F8C85' }}>
      Embedding content<AnimatingDots />
    </span>,
    <span key="generating" style={{ color: '#8F8C85' }}>
      Generating output<AnimatingDots />
    </span>,
  ];

  useEffect(() => {
    // Random completion time between 8-11 seconds to ensure all 3 steps play
    // (3 steps × 2.5 seconds = 7.5 seconds minimum needed)
  ...
  // Render the final content structure based on type
  const finalContent = type === 'badge' ? (
    <span className='inline-block px-2 py-1 rounded-[6px] bg-[#FAFAF9] border border-[#ECEBE9] text-black truncate max-w-full'>
      {value}
    </span>
  ) : (
    <span 
  ...
  if (!isCompleted) {
    return (
      <div className='flex items-center h-full relative'>
        {/* Invisible content to reserve exact space */}
        <div className='invisible' aria-hidden="true">
          {finalContent}
        </div>
        {/* Animation overlay */}
        <div className='absolute inset-0 flex items-center'>
          <TextLoop interval={2.5} transition={{ duration: 0.5 }}>
            {thinkingSteps}
          </TextLoop>
        </div>
      </div>
...
export default function DocumentTable2() {
  ...
  return (
    ...
    >
      <div className='mb-4'>
        <table
          className='border-separate border-spacing-0 rounded-[8px] border border-[#ECEBE9]'
        >
          <thead>
                ...
                {headerGroup.headers.map(header => (
                  <th
                    ...
                    className={`px-3 h-8 text-left font-medium ${
                      ...
                      header.id === 'assignmentProvisionSummary'
                        ? 'w-[280px]'
                        : ''
                  ...
                  return (
                    <td
                      ...
                      className={`${cellPadding} h-8 ${
                        cell.column.id === 'forceMajeureClause'
                          ? 'w-[250px]'
                          : ''
                      } ${
                        cell.column.id === 'agreementParties' ? 'w-[280px]' : ''
                      } ${
                        cell.column.id === 'assignmentProvisionSummary'
                          ? 'w-[280px]'
                          : ''
